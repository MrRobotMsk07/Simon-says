...
class Simon() :
    def __init__(self, master) :
        ...
        self.master.update()
        # Complete any outstanding tkinter tasks.
        ...
        self.idle_colors = ("red", "blue", "green", "olive")
    ...
    def flash(self, color) :
        ...
        # Find the position in the tuple that the specified color is at.
        if self.current_colors[index] == self.idle_colors[index]:
            # Use the position of the color specified to compare if the current color on screen matches the idle one.
            ...
            # Call this function again in 1 seconds time to revert back to the idle color
        else :
            self.current_colors[index] = self.idle_colors[index]
            # Revert the current color back to the idle color.
        self.draw_canvas()
        # Draw the canvas to reflect this change to the player.
    ...
    def check_choice(self) :
        ...
        if(color == self.sequence[self.sequence_position]) :
            ...
            self.sequence[:] = []
            # Empty the list of sequences
            self.sequence.append(choice(self.idle_colors))
            # Add the first sequence of the new game to the list of sequences.
            self.sequence_position = 0
            self.show_sequence()

    def draw_canvas(self) :
        self.rectangle_ids[:] = []
        # Empty out the list of ids.
        self.game_canvas.delete("all") # Clean the frame ready for the new one
        for index, color in enumerate(self.current_colors):
            # Iterate over the colors in the list drawing each of the rectangles their respective place.
            if index <= 1 :
